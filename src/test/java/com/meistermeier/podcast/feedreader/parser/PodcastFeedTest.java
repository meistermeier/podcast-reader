...
public class PodcastFeedTest {
    ...
    public void channelHasCorrectLastBuildDate() {
        assertThat(channel.getLastBuildDate().get(), equalTo(PodcastDate.parsePodcastDate("Tue, 15 Mar 2016 15:21:06 +0000")));
    }

    @Test
    public void channelImageHasCorrectUrl() {
        assertThat(channel.getChannelImage().get().getUrl(), equalTo("http://minkorrekt.de/wp-content/cache/podlove/67/2193777f638039d26446e56cb3e279/methodisch-inkorrekt_original.jpg"));
    }

    @Test
    public void channelImageHasCorrectTitle() {
        assertThat(channel.getChannelImage().get().getTitle(), equalTo("Methodisch inkorrekt"));
    }

    @Test
    public void channelImageHasCorrectLink() {
        assertThat(channel.getChannelImage().get().getLink(), equalTo("http://minkorrekt.de"));
    }

    @Test
    public void channelHasCorrectItem() {
        Item item = channel.getItems().get(0);
        assertThat(item.getLink().get(), equalTo("http://minkorrekt.de/minkorrekt-folge-69-beule-in-der-strumpfhose/"));
        assertThat(item.getPubDate().get(), equalTo(PodcastDate.parsePodcastDate("Tue, 08 Mar 2016 08:00:19 +0000")));
        assertThat(item.getTitle().get(), equalTo("Minkorrekt Folge 69 „Beule in der Strumpfhose“"));
        assertThat(item.getDescription().get(), equalTo("Folge vom 08.03.2016\nHeute mit Gravitationswellen, Ausreden, einem Stickstofflaser, dem India Gadget der Woche, Motivation und Seifenblasen."));
    }

    @Test
    public void itemHasCorrectEnclosure() throws Exception {
        Item item = channel.getItems().get(0);
        assertThat(item.getEnclosure().get().getUrl(), equalTo("http://minkorrekt.de/podlove/file/692/s/feed/c/m4a/methodisch_inkorrekt_Folge_69.m4a"));
        assertThat(item.getEnclosure().get().getLength(), equalTo("168114630"));
        assertThat(item.getEnclosure().get().getType(), equalTo("audio/mp4"));
    }
